...
// Parameters (ajustado para marquee de TV con marco: tvvintage_v4.png, 670x473)
#define HARD_SCAN -8.0
#define CURVATURE vec2(0.025, 0.035)
#define MASK_DARK 0.65
#define MASK_LIGHT 1.3
...
vec4 hook() {
	...
	// Fuera del hueco de la pantalla del marco de TV: pasar el pixel tal cual
	// (no aplicar CRT sobre la carcasa/plastico del marco).
	if (pos.x < SCREEN_MIN.x || pos.x > SCREEN_MAX.x || pos.y < SCREEN_MIN.y || pos.y > SCREEN_MAX.y) {
		return MAIN_tex(pos);
	}

	vec2 rect_size = SCREEN_MAX - SCREEN_MIN;
	vec2 local = (pos - SCREEN_MIN) / rect_size;
	vec2 bent = bend_screen(local);
	vec2 sample_pos = SCREEN_MIN + bent * rect_size;

	vec3 color = tri(sample_pos);
	color += bloom(sample_pos) * BLOOM_AMOUNT;

	#if SHADOW_MASK != 0
	color *= mask(floor(pos * target_size) + vec2(0.5, 0.5));
	#endif

	uint in_bounds = uint(0.0 <= bent.x && bent.x <= 1.0 && 0.0 <= bent.y && bent.y <= 1.0);
